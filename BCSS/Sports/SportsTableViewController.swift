...
class SportsTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        for name in currentSport.coach {
            
            if currentSport.coach[currentSport.coach.count-1] == name {
                coaches += name
                break
            }
            coaches += name
        ...
        //Setting up text
        infoSportController.coachString = coaches
        infoSportController.gradeString = currentSport.grade
